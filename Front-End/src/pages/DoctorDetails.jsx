...
import { Mail, User, Clock, Star, ChevronLeft, ChevronRight, XCircle, Calendar } from 'lucide-react';
...
const DoctorDetails = () => {
  ...
  const { selectedDoctor, status: doctorStatus, error: doctorError } = useSelector((state) => state.doctors);
  const { data: appointments, status: appointmentsStatus, error: appointmentsError } = useSelector((state) => state.appointments);
      ...
      if (result.isConfirmed) {
        const bookingDetails = {
          doctor: {
            ...
            profile_image: selectedDoctor.profile_image || '/default-doctor-image.jpg',
          },
          appointment: {
        ...
        Cookies.set('bookedAppointment', JSON.stringify(bookingDetails), { expires: 7 });
        navigate('/checkout');
      }
    });
  };
        ...
        <div className="container mx-auto py-12 px-4 mt-20">
          <div className="bg-mintD rounded-lg shadow-md mb-8 p-8 text-white">
            <div className="flex flex-col md:flex-row items-center">
              ...
              />
              <div>
                <h1 className="text-4xl font-bold mb-2">{selectedDoctor.name}</h1>
                <div className="flex items-center mb-2">
                  <Clock className="text-mintL mr-1" size={20} />
                  <span className="text-xl">{selectedDoctor.years_of_exprience} years</span>
                </div>
                <p className="text-lg mb-2">{selectedDoctor.specialization}</p>
                <div className="flex flex-wrap gap-4 mt-4">
                  <p className="flex items-center">
                    <Mail className="mr-2 text-mintL" size={18} /> {selectedDoctor.email}
                  </p> 
                  <p className="flex items-center">
                    <User className="mr-2 text-mintL" size={18} /> {selectedDoctor.gender}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-md mb-8 p-8">
            <h2 className="text-2xl font-semibold mb-4">About Dr. {selectedDoctor.name}</h2>
            <p className="text-gray-700">{selectedDoctor.bio || 'No bio available.'}</p>
          </div>

          <div className="bg-white rounded-lg shadow-md mb-8 p-8">
            <h2 className="text-3xl font-bold text-mintD mb-6">Book an Appointment</h2>
            <div className="flex flex-wrap items-center justify-between mb-6">
              <div className="flex items-center space-x-4 mb-4 sm:mb-0">
                <button onClick={() => changeDate(-1)} className="p-2 rounded-full bg-mintL hover:bg-mintD transition-colors">
                  <ChevronLeft size={24} />
                </button>
              ...
              <select
                onChange={(e) => setSelectedTimeSlot(JSON.parse(e.target.value))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredAppointments.map((appointment) => (
                <div key={appointment.id} className={`bg-white rounded-lg shadow-md p-4 ${appointment.is_available ? 'bg-mintL' : 'bg-red-50'}`}>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <p className={`text-sm font-medium ${appointment.is_available ? 'text-green-600' : 'text-red-600'}`}>
                      {appointment.is_available ? 'Available' : 'Not Available'}
                    </p>
                  </div>
                  <button
                    onClick={() => handleBookAppointment(appointment)}
                    className={`w-full py-2 px-4 rounded-md text-white ${appointment.is_available ? 'bg-mintD hover:bg-mint' : 'bg-gray-400 cursor-not-allowed'} transition-colors`}
          ...
          <div className="bg-mintL rounded-lg shadow-md p-8">
            <h2 className="text-3xl font-bold text-mintD mb-6">Feedback for Dr. {selectedDoctor.name}</h2>
            ...
            {feedbackList.length > 0 ? (
              <div className="space-y-4">
                {feedbackList.map((item) => (
                  <div key={item.feedback_id} className="bg-white rounded-lg shadow-md p-4">
                    <div className="flex items-center mb-2">
                      <div className="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center mr-3">
                        <span className="text-xl font-semibold text-gray-600">{item.patient_name.charAt(0)}</span>
                      </div>
                      <div>
